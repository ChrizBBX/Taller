...
function EstadosDetails() {
  ...
  useEffect(() => {
    const EstadoCivilSeleccionado = JSON.parse(localStorage.getItem('EstadoCivilSeleccionado'));
    if (EstadoCivilSeleccionado) {
      setEstadosCiviles(EstadoCivilSeleccionado)
      setEstaciviID(EstadoCivilSeleccionado.estacivi_ID)
      setestaciviNombre(EstadoCivilSeleccionado.estacivi_Nombre)
    }
  }, [Actualizar]);
        ...
        <div className='card-body'>
          <div className='row'>
            <div className='form-group col-6'>
              <h4>ID</h4>
              <label>{EstadosCiviles.estacivi_ID}</label>
            </div>
            <div className='form-group col-6'>
              <h4>Servicio</h4>
              <label>{EstadosCiviles.estacivi_Nombre}</label>
            </div>
          </div>
          <div className="row">
            ...
            <div className="col-3">
              <CButton color="info" onClick={() => { navigate('/EstadosCiviles') }}>Regresar</CButton>
            </div>
            <div style={{ height: 20 }}></div>
          </div>
          <div className="card">
            ...
            <table className="table table-striped">
              ...
              <tbody>
                <tr>
                  ...
                  <td>{EstadosCiviles.estacivi_UserCreacion_Nombre}</td>
                  <td>{EstadosCiviles.estacivi_FechaCreacion}</td>
                </tr>
                <tr>
                  <td>Modificacion</td>
                  <td>{EstadosCiviles.estacivi_UserModificacion_Nombre}</td>
                  <td>{EstadosCiviles.estacivi_FechaModificacion}</td>
                </tr>
              </tbody>
            </table>
          </div>
